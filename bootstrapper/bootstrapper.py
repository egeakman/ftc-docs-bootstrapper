...
class Bootstrapper:
    def __init__(self, language: str, branch: str = "main") -> None:
        ...
        self.ftcdocs_repo = f"{self.translation_repo}/venv/ftcdocs"
        self.readme_url = "https://raw.githubusercontent.com/egeakman/ftc-docs-bootstrapper/master/bootstrapper/data/README.md"
        self.gitignore_url = "https://raw.githubusercontent.com/egeakman/ftc-docs-bootstrapper/master/bootstrapper/data/.gitignore"
        self.makefile_url = "https://raw.githubusercontent.com/egeakman/ftc-docs-bootstrapper/master/bootstrapper/data/Makefile"
    ...
    def create_dirs(self) -> None:
        ...
        os.makedirs(self.ftcdocs_repo, exist_ok=True)
        print("✅")

    def setup_ftcdocs_repo(self) -> None:
        if not os.path.exists(f"{self.ftcdocs_repo}/.git") and not os.path.isdir(
            f"{self.ftcdocs_repo}/.git"
        ):
            logger.info("Cloning ftcdocs repo...")
            subprocess.run(
                [
                    "git",
                    "clone",
                    "https://github.com/FIRST-Tech-Challenge/ftcdocs.git",
                    self.ftcdocs_repo,
                    "-q",
                ],
                check=True,
        ...
        subprocess.run(
            ["git", "-C", self.ftcdocs_repo, "checkout", self.branch, "-q"], check=True
        )
        subprocess.run(
            ["git", "-C", self.ftcdocs_repo, "pull", "--ff-only", "-q"], check=True
        )
    ...
    def setup_translation_repo(self) -> None:
        ...
        files = glob.glob(f"{self.ftcdocs_repo}/pot/**/*.pot", recursive=True)
        ...
        for file in files:
            with open(file, "r", encoding="utf-8") as f:
                contents = f.read()
                contents = re.sub("^#: .*docs/source/", "#: ", contents, flags=re.M)
    ...
    def create_makefile(self) -> None:
        ...
        head = (
            subprocess.run(
                ["git", "-C", self.ftcdocs_repo, "rev-parse", "HEAD"],
                stdout=PIPE,
                check=True,
            )
            .stdout.strip()
